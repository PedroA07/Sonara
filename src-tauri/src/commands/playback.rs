...
pub fn get_queue(db: State<Db>) -> AppResult<Vec<Track>> {
    ...
    let mut stmt = conn.prepare(
        "SELECT t.id, t.title, t.file_path, t.duration, t.track_no, t.disc_no, t.year, t.genre, t.album_id, t.bitrate, t.format, t.gain
         FROM queue_item q JOIN track t ON t.id = q.track_id ORDER BY q.position",
    )?;
    let rows = stmt
        .query_map([], |r| {
            Ok(Track {
