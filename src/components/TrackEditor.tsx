...
export default function TrackEditor({
  ...
  const pickCover = async () => {
    if (!single) return;
    try {
      const img = await open({ multiple: false, filters: [{ name: "Imagem", extensions: ["jpg", "jpeg", "png"] }] });
      if (img && typeof img === "string") await api.setTrackCover(single.id, img, writeFile);
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-panel rounded-2xl p-6 w-[480px] max-w-[90vw]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <Field label="Artista" value={artist} onChange={setArtist} placeholder={single ? "" : "aplicar a todas"} />
          <Field label="Álbum" value={album} onChange={setAlbum} placeholder={single ? "" : "aplicar a todas"} />
          ...
          {single && (
            <button onClick={pickCover} className="text-sm text-brand hover:underline">Escolher capa…</button>
          )}
